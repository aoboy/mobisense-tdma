 ...
 * Version: 10.7
 * $Id: mobmac_ipv6.c,v 1.1.1.1 2010-12-17 15:54:01 aogocvs Exp $
 *----------------------------------------------------------------
...
((uint8_t *)addr)[1], ((uint8_t *)addr)[2], ((uint8_t *)addr)[3], ((uint8_t *)addr)[4], 
        ((uint8_t *)addr)[5], ((uint8_t *)addr)[6], ((uint8_t *)addr)[7])
#endif
#else
#define PRINTF(...)
#if WITH_UIP6
...
/*----------------------------------------------------------------------------*/
//extern  rtimer_clock_t mobisense_system_time;
/*----------------------------------------------------------------------------*/

#define NORM_CLOCK(a) ((rtimer_clock_t)(a%65536UL))
...
/**-------------------------------------------------------------*/
/*each slot request response message contains the requester id and the
 *respective offset.*/
typedef struct schedule{
    u8_t    dl_sched;         /*aa.bbb.ccc n_bcast_ts.dl_offset.dl_ts*/
    u8_t    ts_offset;        /*slot Index/factor 0<= P1 < Num_neighs*/
    u8_t    assig_slots;      /*assigned slots*/
...
sink_schedule_t sink_slots_tbl[] = {
    { 0,  0,  64}, /*0*/
    { 28, 29, 31}, /*1*/
    { 29, 29,  8}, /*2*/
    { 29, 30,  7}, /*3*/
...
typedef struct{ 
  rtimer_clock_t   ad_t0;  /**admission request time*/
  rtimer_clock_t   ad_tn;  /**admission response time_n*/
  rtimer_clock_t   hv_t0;  /**slot_release time_0*/
  rtimer_clock_t   hv_tn;  /**readimission response time*/   
  uint16_t   tx_packets;  
  #if !WITH_UIP6
  uint8_t    rp_type;
  uint8_t    eff_d_rate;
  rtimer_clock_t   nc_t0;  /**boot time*/
  rtimer_clock_t   nc_tn;  /**last admission response time*/
  uint16_t   tx_bytes;
  #endif
...
#define SLOT_MSG_LEN   sizeof(schedule_t)
...
int PWR_DBM(){
    int recv_rssi = cc2420_last_rssi;
...
static volatile uint16_t n_tx_bytes   = 0;
static volatile uint16_t n_tx_packets = 0;
...
static volatile uint8_t node_is_admitted      = 0;
...
static char uplink_transmitter(struct rtimer *t, void* ptr) {
  uint8_t ret;
   ...
   //next_to_send_ul != last_queued_ul
    while ((next_to_send_ul != last_queued_ul ) && (!ul_b_full) 
                          && TIME_DIFF((RTIMER_NOW() + GUARD_TIME), ul_tx_tn)) {
        ...
        if(pkt_len){
            ...
            radio->send((void*)ul_queue[next_to_send_ul], (pkt_len + RDC_HDR_LEN));
            ...
            my_clock_t gtime = RTIMER_NOW() + HALF_MSEC;
            while(TIME_DIFF((RTIMER_NOW()), gtime)); /*wait for packet to be flushed*/
...
static char downlink_transmitter(struct rtimer *t, void *ptr) {
    ...
    for (k = 0, idx = 0;  k < MAX_CHILDS  ; k++) {
        ...
        if (nodes->state == ACTIVE) {
            schedule_t *msg = (schedule_t*) (b_ptr + idx * SLOT_MSG_LEN);
            ...
            if (!rimeaddr_cmp(&(hdr.src), &rimeaddr_node_addr) &&
                ...
                if ((dl_buf_state != NODE_WRITE_BUF) && (dl_b_full == 0)) {
                    ...
                    *hops = *hops + 1;
                    ...
                    /*copy the portion of the received packet to the buffer */
                    //memcpy(dl_queue[last_queued_dl], &msg, sizeof (rdc_msg_t));
                    memcpy(dl_queue[last_queued_dl], &msg, len);
                    last_queued_dl = (last_queued_dl + 1) % DL_QUEUE_SIZE;
                    if (last_queued_dl == next_to_send_dl) {
                        dl_b_full = 1;
                    }
	    ...
	    PRINTF("sched_len: %d\n", schedule_len);
            ...
            if (hdr.pld_len > SYNCH_MSG_LEN) {
                ...
                for (k = 0; k < schedule_len; k += SLOT_MSG_LEN) {
                    ...
                    schedule_t* slot = (schedule_t*) (b_ptr + k);
...
/**---------------------------------------------------------------------------*/
#if NODE_IS_MOBILE 
uint8_t mobile_is_admitted(){
    return (node_is_admitted == 1)? 1:0;
}
/**---------------------------------------------------------------------------*/
#if  RATE_CONTROL
